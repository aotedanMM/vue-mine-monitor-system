<template>
  <div>
    <print-person-track-table :list-data="historyListData"/>
    ...
    <el-table
      ...
      <el-table-column label="出入井" width="200px">
        <template slot-scope="scope">
          <span v-if="scope.row.isOre === 0" class="m_track2"> 井下</span>
          <span v-if="scope.row.isOre === 1" class="m_track1"> 入井</span>
          <span v-if="scope.row.isOre === 2" class="m_track3"> 出井 </span>
          <span v-if="scope.row.isOre === 3" class="m_track4"> 重点区域</span>
          <span v-if="scope.row.isOre === 4" class="m_track5"> 限制区域</span>
...
import PrintPersonTrackTable from './printPersonTrackTable'
export default {
  name: 'PersonTrackHistoryList',
  components: { PrintPersonTrackTable },
  data() {
    return {
      pickerOptions: {
        disabledDate(time) {
      ...
      historyTrackQuery: { total: 0, pageSize: 31, currentTime: '', staffId: null, startPage: 1 }
    }
  },
  watch: {
    timeRange() {
