<template>
  <div id="printTable1">
    <span class="printTitle">特种作业人员工作异常</span>
    <el-table
      id="printTest"
      :data="listData"
      fit
    ...
    >
      ...
      <el-table-column label="指派区域" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.partitionName }}</span>
        </template>
      </el-table-column>
      <el-table-column label="指派时间" width="120">
        <template slot-scope="scope">
          <span>{{ scope.row.assignTime | formatDate }}</span>
        </template>
      </el-table-column>
      <el-table-column label="指派内容" width="120">
        <template slot-scope="scope">
