...
export default {
  ...
  methods: {
    // 开始播放警告气体提示音
    startPlaySingleVoice() {
      this.singlePlayer = document.getElementById('singleVoicePlayerId')
      // 判断是否为第一次播放,如果为第一次播放则不设置延时
      if (this.firstPlayFlag) {
        this.playSingleVoice()
        this.firstPlayFlag = false
      } else {
        this.playPromise = this.singlePlayer.play()
        if (this.playPromise) {
          this.playPromise.then(() => {
            ...
            }, this.singlePlayer.duration * 1000) // audio.duration 为音频的时长单位为秒
          }).catch((e) => {
          })
        }
      }
    ...
    playSingleVoice() {
      if (this.singlePlayer.paused) {
        this.singlePlayer.load()
