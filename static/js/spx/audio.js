(function(global) {
  function readArrayBuffer(src, callback) {
    ...
    xhr.onload = function(e) {
      if (e.target.status == 200) { return callback(e.target.response) }
    }
  ...
  function checkAudioElements(element) {
    for (let i = 0; i < element.length; ++i) {
      // audioElementDecode(element[i], element[i].src)
    }
  ...
  //
    ...
    // element.error.code == MediaError.MEDIA_ERR_DECODE) {
    //   console.warn("Couldn't decode media, trying with speex")
    //   audioElementDecode(element, element.src)
    // }
  }
